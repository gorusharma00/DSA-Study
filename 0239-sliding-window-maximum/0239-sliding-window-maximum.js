...
var maxSlidingWindow = function(nums, k) {
    ...
    for(let i = 0; i < k; i++){
        if(dq.isEmpty()){
            // id deque is empty we can directly add element
            dq.addAtBack(i);
        }else{
            while(!dq.isEmpty() && nums[i] > nums[dq.getBack()]){
                // till the time incoming element > element at back of the queue
                // we will remove elment from back
                dq.removeAtBack();
            }
            dq.addAtBack(i);
        }
    ...
    result.push(nums[dq.getFront()]);  // because we always maintaining our max element is at front;

    for(let i = k; i < nums.length; i++){
        // remove any element that is not in the proximity of window
        while(!dq.isEmpty() && dq.getFront() <= i - k){ // i - k means we have to calculate that element that is discarded from left
            dq.removeAtFront();
        }
